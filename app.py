...
def generate_random_name(name_prefix):
    ...
    return name_prefix + 'Zyroxx'.join(random.choice(characters) for _ in range(6)).upper()

def generate_custom_password():
    characters = string.ascii_letters + string.digits
    random_part = 'zyroxx'.join(random.choice(characters) for _ in range(9)).upper()
    return f"Zyroxx-{random_part}-CORE"
